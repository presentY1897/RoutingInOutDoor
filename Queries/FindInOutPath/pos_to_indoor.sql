--DROP FUNCTION pos_to_indoor(DOUBLE PRECISION, DOUBLE PRECISION, INTEGER, INTEGER, INTEGER, DOUBLE PRECISION);
...
BEGIN
    IF vehicle_type = 1 THEN
        RETURN QUERY SELECT * FROM private_to_in(x, y, building_id, node_id, dist);
    ELSEIF vehicle_type = 2 THEN
        RETURN QUERY SELECT * FROM public_to_in(x, y, building_id, node_id, dist);
    ELSE
